/*
================================================================================
SCRIPT DE PRUEBA ETL (MIGRACIÓN TRANSACCIONAL)
Descripción: Este script se puede ejecutar de principio a fin para verificar 
             que la transformación y reestructuración del modelo dimensional 
             se completen sin errores. 
             Al finalizar con ROLLBACK, la base de datos conservará su estado 
             original sin alterar ningún dato en producción.
================================================================================
*/

-- Consulta a los datos originales antes del etl
SELECT *
FROM datos_originales.tarjetas_de_crédito_y_débito tdcyd
...
ALTER TABLE datos_originales."tarjetas_de_crédito_y_débito"  RENAME TO fact_tarjetas_credito_debito;
 
-- 1.2 Normalizar nombres de columnas críticas respetando las comillas de la fuente original
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "TIPOENTIDAD" TO id_tipo_entidad;
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "CODIGOENTIDAD" TO codigo_entidad;
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "FECHACORTE" TO fecha_corte;
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "PERSONA_NATURAL" TO persona_natural;
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "PERSONA_JURIDICA" TO persona_juridica;
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "TOTAL_TARJETAS" TO total_tarjetas;

-- 1.3 Limpieza: Eliminar columnas redundantes o sin contexto relacional
ALTER TABLE datos_originales.fact_tarjetas_credito_debito DROP COLUMN codigo_entidad;

-- 2.1 DIMENSIÓN: TIPO DE ENTIDAD (Mapeo según normativa SFC)
CREATE TABLE datos_originales.dim_tipo_entidad (
    id_tipo_entidad INT PRIMARY KEY,
    nombre_tipo_entidad VARCHAR(100)
);

INSERT INTO datos_originales.dim_tipo_entidad (id_tipo_entidad, nombre_tipo_entidad)
VALUES 
...
CREATE TABLE datos_originales.dim_nombre_entidad AS
SELECT DISTINCT "NOMBREENTIDAD" AS nombre_entidad
FROM datos_originales.fact_tarjetas_credito_debito
ORDER BY nombre_entidad ASC;

ALTER TABLE datos_originales.dim_nombre_entidad 
ADD COLUMN id_nombre_identidad SERIAL PRIMARY KEY;


-- 2.3 DIMENSIÓN: UCA (Unidad de Captura)
CREATE TABLE datos_originales.dim_uca AS
SELECT DISTINCT "COD_UCA" AS cod_uca, "NOMBRE_UCA" AS nombre_uca
FROM datos_originales.fact_tarjetas_credito_debito;

-- 2.3.1 Normalización de nombre del atributo "COD_UCA"
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "COD_UCA" TO cod_uca;

-- Limpieza en Hechos tras aislar la dimensión UCA
ALTER TABLE datos_originales.fact_tarjetas_credito_debito DROP COLUMN "NOMBRE_UCA";


-- 2.4 DIMENSIÓN: CONCEPTO (Estructurada a partir del atributo Subcuenta)
CREATE TABLE datos_originales.dim_concepto (
    id_concepto INT PRIMARY KEY,
    descripcion_concepto VARCHAR(255) NOT NULL
);

INSERT INTO datos_originales.dim_concepto (id_concepto, descripcion_concepto) VALUES
...
CREATE TABLE datos_originales.dim_descr_detallada AS
SELECT DISTINCT "DESCRIPCION" AS descripcion 
FROM datos_originales.fact_tarjetas_credito_debito;

ALTER TABLE datos_originales.dim_descr_detallada 
ADD COLUMN id_descr_detallada SERIAL PRIMARY KEY;

-- 3.1 Vinculación con dim_nombre_entidad
ALTER TABLE datos_originales.fact_tarjetas_credito_debito ADD COLUMN id_nombre_identidad INT;

UPDATE datos_originales.fact_tarjetas_credito_debito ftcd 
SET id_nombre_identidad = dne.id_nombre_identidad
FROM datos_originales.dim_nombre_entidad dne
WHERE dne.nombre_entidad = ftcd."NOMBREENTIDAD";

ALTER TABLE datos_originales.fact_tarjetas_credito_debito DROP COLUMN "NOMBREENTIDAD";


-- 3.2 Vinculación con dim_concepto (Reemplazo del campo subcuenta)
ALTER TABLE datos_originales.fact_tarjetas_credito_debito RENAME COLUMN "SUBCUENTA" TO id_concepto;


-- 3.3 Vinculación con dim_descr_detallada
ALTER TABLE datos_originales.fact_tarjetas_credito_debito ADD COLUMN id_descr_detallada INT;

UPDATE datos_originales.fact_tarjetas_credito_debito ftcd 
SET id_descr_detallada = ddd.id_descr_detallada
FROM datos_originales.dim_descr_detallada ddd
WHERE ftcd."DESCRIPCION" = ddd.descripcion;

ALTER TABLE datos_originales.fact_tarjetas_credito_debito DROP COLUMN "DESCRIPCION";
...
ALTER TABLE datos_originales.fact_tarjetas_credito_debito 
ADD COLUMN id_tipo_metrica INT;

-- Crear tabla de dimensión métrica
CREATE TABLE datos_originales.dim_metrica (
    id_tipo_metrica INT PRIMARY KEY,
    tipo_metrica VARCHAR(50)
);

INSERT INTO datos_originales.dim_metrica (id_tipo_metrica, tipo_metrica)
VALUES 
    (1, 'Monto financiero'),
    (2, 'Cantidad / Conteo');

-- Ampliación de atributos en dim_metrica
ALTER TABLE datos_originales.dim_metrica 
...
UPDATE datos_originales.dim_metrica
...
UPDATE datos_originales.fact_tarjetas_credito_debito AS ftcd 
SET id_tipo_metrica = 2
FROM datos_originales.dim_concepto AS dc 
...
UPDATE datos_originales.fact_tarjetas_credito_debito 
...
DROP TABLE datos_originales.dim_concepto;

ALTER TABLE datos_originales.fact_tarjetas_credito_debito 
DROP COLUMN id_concepto;

-- Renombramiento de dim_descr_detallada a dim_descripcion
ALTER TABLE datos_originales.dim_descr_detallada RENAME TO dim_descripcion;

ALTER TABLE datos_originales.fact_tarjetas_credito_debito 
RENAME COLUMN id_descr_detallada TO id_descripcion;

ALTER TABLE datos_originales.dim_descripcion 
RENAME COLUMN id_descr_detallada TO id_descripcion;
...
CREATE TABLE datos_originales.dim_fecha (
...
INSERT INTO datos_originales.dim_fecha (
    id_fecha,
    fecha_corte,
    dia_num,
    mes_num,
...
SELECT DISTINCT 
    ...
    END AS mes_name
FROM datos_originales.fact_tarjetas_credito_debito AS ftcd
...
WITH ultima_fecha AS (
    ...
    FROM datos_originales.dim_fecha AS df
)
UPDATE datos_originales.dim_fecha AS df
...
-- Comparamos las tablas de pruebas con la tabla ya normalizada Y una dimension cualquiera
SELECT *
FROM datos_originales.fact_tarjetas_credito_debito TABLA_PRUEBA
ORDER BY VALOR DESC
LIMIT 10;
...
SELECT *
FROM datos_tarjetas.dim_fecha df
ORDER BY id_fecha DESC
LIMIT 10;

SELECT *
FROM datos_originales.dim_fecha df 
ORDER BY id_fecha DESC
LIMIT 10;
